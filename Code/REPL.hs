...
process (Help) = do
    ...
    liftIO $ putStrLn ("  - toString(x) {Convert x to string}")
    liftIO $ putStrLn ("  - toInt(x) {Convert x to integer}")
    liftIO $ putStrLn ("  - toFloat(x) {Convert x to float}")
    ...
    liftIO $ putStrLn ("  - def function_name(var1, var2) { var1 - var2 } {Define a function with variables and commands to execute}")
